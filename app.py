...
if st.sidebar.button("Save Entry"):
    if not event_data:
        ...
        for event, sev in event_data.items():
            # FIXED: Added the required 'else' block for categorization
        ...
        if selected_item != "-- Select --":
            ...
            with col_e:
                ...
                if st.button("Update Log"):
                    df.at[item_idx, 'Event'] = ", ".join(new_events)
                    df.at[item_idx, 'Severity'] = new_sev
                    df.at[item_idx, 'Notes'] = new_note
                    # Ensure Type updates correctly
                    df.at[item_idx, 'Type'] = "Symptom" if new_events[0] in symptom_options else "Trigger"
